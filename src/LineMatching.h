...
static Mat empty_LineMatching = Mat();
...
private:
	...
	void description(vector<strFanSection> &strFanSect,  Mat gMag,  Mat gDir, bool isOutputDesMat = false, Mat &desMat = empty_LineMatching);
	...
	void description_fans(vector<strFanSection> &vstrFanSection,  Mat gMag,  Mat gDir, bool isOutputDesMat = false, Mat &desMat = empty_LineMatching);
	...
	void uniqueChk(Mat inMat, Vec3i regulation, vector<int> &vKeptIdx, Mat &outMat = empty_LineMatching);
